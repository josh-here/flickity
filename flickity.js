...
if (flktySlider) {
  ...
  flktyObserver.observe(document.querySelector(".item .item-image .is-last"));
  ...
  /* iOS 12-14 fix */
  var tapArea, startX;
  for (var item of ((tapArea = document.querySelectorAll(".featured-wrap")),
  (startX = 0),
  tapArea))
    (item.ontouchstart = function (t) {
      startX = t.touches[0].clientX;
    }),
      (item.ontouchmove = function (t) {
        Math.abs(t.touches[0].clientX - startX) > 5 &&
          t.cancelable &&
          t.preventDefault();
      });
}
